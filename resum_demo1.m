% test Taylor series resummation via conformal map, for Parcollet/CCQ
% Barnett 7/2/19. Try SCtoolbox 7/29/19.
clear; verb=1;
...
ztarg = 3;                           % desired z target
ftrue = f(ztarg)                     % cheating, true ans

map = 'p';                       % map type: 'a' RHP parabola, 'p' SC poly
...
elseif map=='p'  % arb polygon via SC toolbox
  ...
  w = @(z) evalinv(g,z);   % w(z), our map. Note SC evalinv is slow
end
...
if verb, figure(2); imagesc(0:p,0:p,log10(abs(L))); axis equal; colorbar;
  title('log_{10} L_{mn}'); v=caxis; v(1)=-2; caxis(v);
  r2 = 0.5; L2 = matrixfrominvmap(iw,p,r2);   % check stability wrt r
...
if verb    % plot both planes, first z then w...
  ...
  elseif map=='p'
    ...
    plot(g); hold on    % use overloaded SC plot method
  end
  axis equal; axis(3*[-1 1 -1 1]);
  plot(real(fsing),imag(fsing),'r*'); plot(0,0,'ko')
  plot(real(ztarg),imag(ztarg),'k.'); title('z plane');
  plot(iw(r*exp(2i*pi*(1:Nc)/Nc)),'m.');   % colloc pts used to get L
  %
  subplot(2,1,2); plot(exp(1i*2*pi*(1:100)/100),'r-');  % check is unit circ
  ...
  plot(r*exp(2i*pi*(1:Nc)/Nc),'m.');
  axis tight equal; title('w plane');
end

% do the eval
c = L*fn;        % get w-plane Taylor coeffs
ftarg = sum(c.'.*wtarg.^(0:p))
e = (ftarg-ftrue)/ftrue;
fprintf('ztarg=(%.3g,%.3g): f rel err = %.3g\n',real(ztarg),imag(ztarg),e)
